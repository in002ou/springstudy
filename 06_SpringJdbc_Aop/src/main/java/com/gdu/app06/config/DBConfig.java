...
public class DBConfig {
	...
	// JdbcTemplate Bean (Connection, PreparedStatment, ResultSet을 이용해서 동작하는 스프링 클래스)
	@Bean
	...
	/*
	 	AOP 관련 핵심 용어 3가지
	 	1. 조인포인트(JoinPoint) : AOP를 적용시킬 수 있는 메소드 전체		(전체집합개념 ex - 목록, 상세, 삽입, 수정, 삭제)
	 	2. 포인트컷(PointCut)	 : 조인포인트 중에서 AOP를 동작시킬 메소드	(조인포인트의 부분집합 개념 ex - 삽입, 수정, 삭제)
	 	3. 어드바이스(Advice)	 : 포인트컷에 동작시킬 AOP 동작 자체		- 트랜잭션 (ex - 포인트컷의 삽입, 수정, 삭제에 트랜잭션 처리)
	 */
