...
public class FifthServiceImpl implements IFifthService {
	...
	public ResponseEntity<String> search(HttpServletRequest request) {
		...
		try {
			...
			String params = "?query=" + URLEncoder.encode(query, "UTF-8") + "&display=" + display + "&sort=" + sort;
			String apiURL = "https://openapi.naver.com/v1/search/shop.json" + params;
			
			URL url = new URL(apiURL);
			
			// 예측 url의 클래스가 더 높아서 다운캐스팅을 했을거라 추측
			HttpURLConnection con = (HttpURLConnection) url.openConnection();
