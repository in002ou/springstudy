...
public class FirstController {
	...
	// Spring Container에 등록된 firstService Bean을 아래 필드 firstService에 주입해보자. (필드에 @Autowried 추가)

	@Autowired
	private IFirstService firstService;
	
	
	@ResponseBody	// 메소드의 반환 값을 jsp이름이 아닌, 응답하는 값(데이터)이다.(요청한 곳으로 보내는 데이터이다.)
	...
	public Person ajax1(HttpServletRequest request,
						HttpServletResponse response) {				// "Jackson 라이브러리"가 반환값 Person 객체를 자동으로 JSON 데이터로 반환한다.
	...
	public Map<String, Object> ajax2(@RequestParam("name") String name, @RequestParam("age") int age) {
		return firstService.execute2(name, age);		// "Jackson라이브러리"가 반환값 Map을 자동으로 JSON 데이터로 반환한다.
	}
