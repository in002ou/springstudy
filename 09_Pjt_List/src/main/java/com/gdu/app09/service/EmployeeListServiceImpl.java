...
public class EmployeeListServiceImpl implements EmployeeListService {
	...
	public void getEmployeeListUsingPagination(HttpServletRequest request, Model model) {
		...
		// 세션에 있는 recordPerPage를 가져온다. 세션에 없는 경우 recordPerPage=10으로 처리.
		...
		// 파라미터 column이 전달되지 않는 경우 column=EMPLOYEE_ID로 처리
		...
		switch(order) {
		case "ASC": model.addAttribute("order", "DESC"); break;	// 현재 ASC 정렬이므로 다음 정렬은 DESC
