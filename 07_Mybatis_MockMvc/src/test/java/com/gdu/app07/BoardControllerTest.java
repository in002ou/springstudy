...
// 테스트에서 사용할 Bean이 @Component로 생성되었기에 Component-scan이 작성된 servlet-context.xml의 경로를 작성한다.
@ContextConfiguration(locations = {"file:src/main/webapp/WEB-INF/spring/appServlet/servlet-context.xml"})	// 경로 src부터 적는다.
		...
		LOGGER.debug(mockMvc.perform(MockMvcRequestBuilders
				.post("/board/add.do")					// @PostMapping("/board/add.do")
					...
					.param("writer", "테스트작성자"))	// 파라미터
			 			.andReturn()					// 삽입결과
			 			.getFlashMap()					// FlashAttribute로 저장한 결과 호출
			 				.toString());				// 문자열반환
	}
		...
		LOGGER.debug(mockMvc.perform(MockMvcRequestBuilders
				.get("/board/detail.do")		// @GetMapping("/board/detail.do")
					.param("boardNo", "1"))		// 파라미터
						...
						.getModelMap()			// ModelAndView에서 Model을 가져온다.
							.toString());		// Model 데이터를 문자열 처리
	}
	@Test
